...
defmodule AshNeo4j.Type.NxTensorTest do
  ...
  describe "cast_input/2" do
    test "from a nested list, inferring shape and defaulting type to u8" do
      assert {:ok, t} = NxTensor.cast_input([[1, 2, 3], [4, 5, 6]], [])
      assert Nx.to_list(t) == [[1, 2, 3], [4, 5, 6]]
      assert Nx.type(t) == {:u, 8}
    end

    test "errors on an unknown element type" do
      assert {:error, _} = NxTensor.cast_input([1, 2], type: :not_a_type)
    end

    test "from a flat list with an explicit shape" do
      assert {:ok, t} = NxTensor.cast_input([1, 2, 3, 4], shape: [2, 2])
      assert Nx.to_list(t) == [[1, 2], [3, 4]]
    end

    test "coerces to the constrained type" do
      assert {:ok, t} = NxTensor.cast_input([[1, 2]], type: :f32)
      assert Nx.type(t) == {:f, 32}
      assert Nx.to_list(t) == [[1.0, 2.0]]
    end

    test "passes through an %Nx.Tensor{}" do
      assert {:ok, t} = NxTensor.cast_input(Nx.tensor([1, 2, 3]), [])
    ...
    test "errors on rank > 3" do
      assert {:error, _} = NxTensor.cast_input([[[[1]]]], [])
    end

    test "errors on a shape-constraint mismatch" do
      assert {:error, _} = NxTensor.cast_input([[1, 2], [3, 4]], shape: [4])
    end
  end

  describe "dump_storage/2 + cast_stored/2 round-trip" do
    test ":property — flat LIST + shape sidecar; type comes from the constraint" do
      tensor = Nx.tensor([[1, 2, 3], [4, 5, 6]], type: :s32)
      assert {[1, 2, 3, 4, 5, 6], [2, 3]} = NxTensor.dump_storage(tensor, :property)

      assert {:ok, restored} =
               NxTensor.cast_stored(%{data: [1, 2, 3, 4, 5, 6], shape: [2, 3]}, type: :s32, store: :property)
    ...
    test "sub-word types (u8/s8) round-trip through both codecs" do
      for {type, store} <- [{:u8, :property}, {:u8, :packed}, {:s8, :property}, {:s8, :packed}] do
        {:ok, tensor} = NxTensor.cast_input([[1, 2], [3, 4]], type: type)
        {stored, shape} = NxTensor.dump_storage(tensor, store)
        assert {:ok, restored} = NxTensor.cast_stored(%{data: stored, shape: shape}, type: type, store: store)
    ...
    test ":packed of a u8 tensor is one byte per element (the sub-word win)" do
      ...
      {stored, _shape} = NxTensor.dump_storage(tensor, :packed)
    ...
    test ":packed — base64 binary preserves f32" do
      ...
      assert {stored, [2, 2]} = NxTensor.dump_storage(tensor, :packed)
      assert is_binary(stored)

      assert {:ok, restored} = NxTensor.cast_stored(%{data: stored, shape: [2, 2]}, type: :f32, store: :packed)
